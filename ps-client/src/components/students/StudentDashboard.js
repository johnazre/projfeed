      ...
      <Container>
        <Row style={{ marginTop: 30 }}>
          <Col md={{ size: 8, offset: 2 }}>
            <p style={{ fontSize: '1.7em', marginBottom: 0 }}>
              Which project would you like to see the feedback for?
            </p>
            <small>(Click on the corresponding project to proceed)</small>
            <ListGroup>{listOfProjects}</ListGroup>
          </Col>
        </Row>
        ...
        <Row>
          <Col className="text-center">
            <Button
              color="primary"
              onClick={() => this.props.history.push('/students/projects/add')}
            >
              Add New Project
            </Button>
          </Col>
...
function mapStateToProps(state, props) {
  ...
  return {
    projects: state.projects.filter(
      project => project.author_id === state.auth.user.id
    )
  }
}
